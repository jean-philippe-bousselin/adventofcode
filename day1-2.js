...
let amount = modulesMass.reduce((acc, mass) => {
	const x = getFuelRequired(mass);
	return acc + x + getFuelRequiredForFuel(x);
}, 0)


function getFuelRequiredForFuel(fuelAmount) {
	let f = getFuelRequired(fuelAmount)
	console.log('input', f)
	if(f > 0) {
		const x = f + getFuelRequiredForFuel(f)
		console.log(x)
		return x
	} else {
		console.log(f)
