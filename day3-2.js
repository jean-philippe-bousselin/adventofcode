...
function move(from, movement) {
	switch(movement.direction) {
		case 'U': return {...from, y: parseInt(from.y) + parseInt(movement.distance), traveled: from.traveled + movement.distance}
			break;
		case 'D': return {...from, y: parseInt(from.y) - parseInt(movement.distance), traveled: from.traveled + movement.distance}
			break;
		case 'L': return {...from, x: parseInt(from.x) - parseInt(movement.distance), traveled: from.traveled + movement.distance}
			break;
		case 'R': return {...from, x: parseInt(from.x) + parseInt(movement.distance), traveled: from.traveled + movement.distance}
			break;
		default: throw new Error('unknown direction : ' + movement.direction)
	}
}

function calculateWireCoordinates(path) {
	const wirePathString 	  = path
	const wirePathArray 		= wirePathString.split(',').map(x => x.trim());
	const wirePathMovements = wirePathArray.map( (m) => { return {direction: m[0], distance: m.substring(1)} } );
	let from = {x: 0, y: 0, traveled: 0}
	let allCoordinates = []

	wirePathMovements.map(movement => {
		for (let i = 1; i <= movement.distance; i++) {
			from = move(from, {...movement, distance: 1})
			allCoordinates.push(from)
		}
	})
...
function getIntersections(array1, array2) {
	return intersect([array1, array2], c => c.x+'_'+c.y)
}
